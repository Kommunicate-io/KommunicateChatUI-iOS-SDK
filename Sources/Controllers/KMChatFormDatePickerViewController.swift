//
//  KMChatFormDatePickerViewController.swift
...
@objc protocol KMChatDatePickerButtonClickProtocol {
    func confirmButtonClick(position: Int, date: Date, messageKey: String, datePickerMode: UIDatePicker.Mode)
}

class KMChatFormDatePickerViewController: UIViewController, Localizable {
    enum Padding {
        enum ModelView {
    ...
    weak var delegate: KMChatDatePickerButtonClickProtocol?
    var action: String!
    ...
    init(delegate: KMChatDatePickerButtonClickProtocol,
         messageKey: String,
         position: Int,
