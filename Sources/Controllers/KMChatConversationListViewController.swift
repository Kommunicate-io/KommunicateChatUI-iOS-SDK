//
//  KMChatConversationListViewController.swift
...
/// The delegate of an `KMChatConversationListViewController` object.
/// Provides different methods to manage chat thread selections.
public protocol KMChatConversationListDelegate: AnyObject {
    func conversation(
        _ message: KMChatChatViewModelProtocol,
        willSelectItemAt index: Int,
        viewController: KMChatConversationListViewController
    )
}

open class KMChatConversationListViewController: KMChatBaseViewController, Localizable {
    public var conversationViewController: KMChatConversationViewController?
    public var conversationViewModelType = KMChatConversationViewModel.self
    public weak var delegate: KMChatConversationListDelegate?
    ...
    var viewModel = KMChatConversationListViewModel()
    ...
    override open func addObserver() {
        ...
        NotificationCenter.default.addObserver(
            ...
            using: { [weak self] _ in
                ...
                if weakSelf.navigationController?.visibleViewController is KMChatConversationListViewController, weakSelf.configuration.isMessageSearchEnabled, weakSelf.searchBar.searchBar.text == "" {
    ...
    func launchChat(contactId: String?, groupId: NSNumber?, conversationId: NSNumber? = nil) {
        ...
        let viewController: KMChatConversationViewController!
        if conversationViewController == nil {
            viewController = KMChatConversationViewController(configuration: configuration, individualLaunch: false)
    ...
    @objc func compose() {
        let newChatVC = KMChatNewChatViewController(configuration: configuration, viewModel: KMChatNewChatViewModel(localizedStringFileName: configuration.localizedStringFileName))
    ...
    fileprivate func push(conversationVC: KMChatConversationViewController, with viewModel: KMChatConversationViewModel) {
        if let topVC = navigationController?.topViewController as? KMChatConversationViewController {
    ...
    func conversationVC() -> KMChatConversationViewController? {
        return navigationController?.topViewController as? KMChatConversationViewController
    }
}

// MARK: KMCoreMessagesDelegate

extension KMChatConversationListViewController: KMCoreMessagesDelegate {
...
extension KMChatConversationListViewController: KMChatConversationListViewModelDelegate {
...
extension KMChatConversationListViewController: ALMQTTConversationDelegate {
    open func mqttDidConnected() {
        if let viewController = navigationController?.visibleViewController as? KMChatConversationViewController {
    ...
    func isNewMessageForActiveThread(alMessage: KMCoreMessage, vm: KMChatConversationViewModel) -> Bool {
    ...
    open func syncCall(_ alMessage: KMCoreMessage!, andMessageList _: NSMutableArray!) {
        ...
        let viewController = navigationController?.visibleViewController as? KMChatConversationViewController
        if let vm = viewController?.viewModel, vm.contactId != nil || vm.channelKey != nil,
           let visibleController = navigationController?.visibleViewController,
           visibleController.isKind(of: KMChatConversationViewController.self),
        ...
        if let visibleController = navigationController?.visibleViewController,
           visibleController.isKind(of: KMChatConversationListViewController.self) {
    ...
    open func reloadData(forUserBlockNotification userId: String!, andBlockFlag _: Bool) {
        ...
        guard let viewController = navigationController?.visibleViewController as? KMChatConversationViewController else {
            return
        }
    ...
    open func mqttConnectionClosed() {
        ...
        print("Retrying MQTT connection in KMChatConversationListVC after seconds:", String(format: "%.f", intervalSeconds))
...
extension KMChatConversationListViewController: KMChatConversationListTableViewDelegate {
    public func tapped(_ chat: KMChatChatViewModelProtocol, at index: Int) {
...
extension KMChatConversationListViewController: UISearchBarDelegate {
