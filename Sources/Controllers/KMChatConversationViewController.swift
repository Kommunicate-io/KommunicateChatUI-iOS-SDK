//  KMChatConversationViewController.swift
...
open class KMChatConversationViewController: KMChatBaseViewController, Localizable {
    var timerTask = Timer()

    public var viewModel: KMChatConversationViewModel! {
        willSet(updatedVM) {
    ...
    open lazy var navigationBar = KMChatConversationNavBar(configuration: self.configuration, delegate: self)
    ...
    var loadingIndicator = KMChatLoadingIndicator(frame: .zero)
    ...
    fileprivate var audioPlayer = KMChatAudioPlayer()

    fileprivate let moreBar = KMChatMoreBar(frame: .zero)
    ...
    var documentManager = KMChatDocumentManager()
    lazy var photoPicker = KMChatPhotoPicker(
    ...
    open var contextTitleView: KMChatContextTitleView = {
        let contextView = KMChatContextTitleView(frame: CGRect.zero)
    ...
    open var templateView: KMChatTemplateMessagesView?

    open lazy var replyMessageView: KMChatReplyMessageView = {
        let view = KMChatReplyMessageView(frame: CGRect.zero, configuration: configuration)
    ...
    public init(configuration: KMChatConfiguration,
    ...
    public required init(configuration _: KMChatConfiguration) {
...
//        setupConstraints()
    ...
    public func prepareBusinessHoursInfoView(message: String, isVisible: Bool = false) {
        let appsetting = KMChatAppSettingsUserDefaults()
    ...
    @objc func conversationInfoViewTap(_: UITapGestureRecognizer) {
        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.conversationInfoClick, data: nil)
    }
...
//        bottomConstraint?.isActive = true
    ...
    private func prepareTable() {
        ...
        tableView.register(KMChatMyMessageCell.self)
        tableView.register(KMChatFriendMessageCell.self)
        tableView.register(KMChatMyPhotoPortalCell.self)
        tableView.register(KMChatMyPhotoLandscapeCell.self)

        tableView.register(KMChatFriendPhotoPortalCell.self)
        tableView.register(KMChatFriendPhotoLandscapeCell.self)

        tableView.register(KMChatMyVoiceCell.self)
        tableView.register(KMChatFriendVoiceCell.self)
        tableView.register(KMChatInformationCell.self)
        tableView.register(KMChatMyLocationCell.self)
        tableView.register(KMChatFriendLocationCell.self)
        tableView.register(KMChatMyVideoCell.self)
        tableView.register(KMChatFriendVideoCell.self)
        tableView.register(KMChatMyGenericCardMessageCell.self)
        tableView.register(KMChatFriendGenericCardMessageCell.self)
        tableView.register(KMChatFriendMessageQuickReplyCell.self)
        tableView.register(KMChatMyMessageQuickReplyCell.self)
        tableView.register(KMChatMyMessageButtonCell.self)
        tableView.register(KMChatFriendMessageButtonCell.self)
        tableView.register(KMChatMyMessageListTemplateCell.self)
        tableView.register(KMChatFriendMessageListTemplateCell.self)
        tableView.register(KMChatMyDocumentCell.self)
        tableView.register(KMChatFriendDocumentCell.self)
        ...
        tableView.register(KMChatMyLinkPreviewCell.self)
        tableView.register(KMChatFriendLinkPreviewCell.self)
        tableView.register(KMChatMyFormCell.self)
        tableView.register(KMChatFriendFormCell.self)
    ...
    private func prepareChatBar() {
        ...
        chatBar.action = { [weak self] action in
            ...
            case .showImagePicker:
                KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.attachmentOptionClicked, data: ["attachmentType": "Image/Video"])
                if #available(iOS 14, *), weakSelf.configuration.isNewSystemPhotosUIEnabled {
                    weakSelf.photoPicker.openGallery(from: weakSelf)
                } else {
                    guard let vc = KMChatCustomPickerViewController.makeInstanceWith(delegate: weakSelf, and: weakSelf.configuration)
                    else {
                        return
                    }
            ...
            case .showLocation:
                KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.locationButtonClicked, data: nil)
                let storyboard = UIStoryboard.name(storyboard: UIStoryboard.Storyboard.mapView, bundle: Bundle.km)

                guard let nav = storyboard.instantiateInitialViewController() as? KMChatBaseNavigationViewController else { return }
                guard let mapViewVC = nav.viewControllers.first as? KMChatMapViewController else { return }
                mapViewVC.delegate = self
                mapViewVC.setConfiguration(weakSelf.configuration)
                self?.present(nav, animated: true, completion: {})
            case let .cameraButtonClicked(button):
                KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.cameraButtonClicked, data: nil)
                guard let vc = KMChatCustomCameraViewController.makeInstanceWith(delegate: weakSelf, and: weakSelf.configuration)
                else {
                    button.isUserInteractionEnabled = true
                    return
                }
            ...
            case .showDocumentPicker:
                KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.attachmentOptionClicked, data: ["attachmentType": "Document"])
    ...
    @objc open func pushNotification(notification: NSNotification) {
        print("Push notification received in KMChatConversationViewController: ", notification.object ?? "")
    ...
    public func showTypingLabel(status: Bool, userId: String) {
        ...
        if status {
            ...
            if botDelayTime > 0 && contact.roleType == NSNumber(value: AL_BOT.rawValue) {
    ...
    func isCellVisible(identifier: String) -> Bool {
        ...
        let cell = tableView.cellForRow(at: IndexPath(row: 0, section: index)) as? KMChatChatBaseCell<KMChatMessageViewModel>
        return cell != nil
    }

    func messageAvatarViewDidTap(messageVM: KMChatMessageViewModel, indexPath _: IndexPath) {
        // Open chat thread
        ...
        let vm = KMChatConversationViewModel(contactId: receiverId, channelKey: nil, localizedStringFileName: configuration.localizedStringFileName)
        let conversationVC = KMChatConversationViewController(configuration: configuration, individualLaunch: true)
    ...
    func scrollTo(message: KMChatMessageViewModel) {
        guard
        ...
        guard let indexPath = viewModel.getIndexpathFor(message: actualMessage)
            ...
            let controller = KMChatReplyController(messageKey: replyId, configuration: configuration)
    ...
    func quickReplySelected(
        ...
        title: String,
        template: [[String: Any]],
        message: KMChatMessageViewModel,
        ...
        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.richMessageClick, data: ["conversationId": conversationId.stringValue, "action": [:], "type": "quickreply"])
    }
    ...
    func richButtonSelected(index: Int,
        ...
        if let conversationId = message.channelKey {
            KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.richMessageClick, data: ["conversationId": conversationId.stringValue, "action": action, "type": type])
        }
    ...
    func messageButtonSelected(
        ...
        guard
            ...
            /// Submit Button
            KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.richMessageClick, data: ["action": selectedButton, "type": "submit"])
        ...
        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.richMessageClick, data: ["action": selectedButton, "type": "link"])
    ...
    func listTemplateSelected(element: KMListTemplate.Element?, defaultText: String?, action: KMListTemplate.Action?) {
        ...
        guard let action = action, let type = action.type  else {
            KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.richMessageClick, data: ["conversationId": viewModel.channelKey?.stringValue ?? "Conversation ID not Present", "action": element ?? "Element not Present", "type": ""])
            print("Type not defined for action")
            return
        }

        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.richMessageClick, data: ["conversationId": viewModel.channelKey?.stringValue ?? "Conversation ID not Present", "action": element ?? "Element not Present", "type": type])
        switch type {
    ...
    func cardTemplateSelected(tag: Int, title: String, template: KMCardTemplate, message: KMChatMessageViewModel) {
        guard
        ...
        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.richMessageClick, data: ["action": action, "type": action.type])
    ...
    func openRichMessageImageView(imageUrl: URL) {
        ...
        guard let nav = storyboard.instantiateInitialViewController() as? KMChatBaseNavigationViewController,
              let vc = nav.viewControllers.first as? KMChatPreviewImageViewController
        else {
            return
        }
        vc.configuration = configuration
        vc.viewModel = KMChatPreviewImageViewModel(imageUrl: imageUrl, localizedStringFileName: configuration.localizedStringFileName)
    ...
    func showParticipantListChat() {
        ...
        if let vc = storyboard.instantiateViewController(withIdentifier: "KMChatCreateGroupViewController") as? KMChatCreateGroupViewController {
            vc.configuration = configuration
    ...
    private func configurePropertiesWith(configuration: KMChatConfiguration) {
    ...
    private func submitButtonResponse(request: URLRequest) {
        ...
        group.notify(queue: .main) {
            ...
            let vc = KMChatWebViewController(htmlString: data, url: url, title: "")
    ...
    func formSubmitButtonSelected(formSubmitData: FormDataSubmit?, messageModel: KMChatMessageViewModel, isButtonClickDisabled: Bool) {
        guard isConversationResolvedAndReplyEnabled() else { return }
    ...
    func sendPostSubmittedFormDataAsMessage(message: String?, messageModel: KMChatMessageViewModel, postFormData: [String: Any], chatContextData: [String: Any]!) {
    ...
    func setRichMessageKitTheme() {
        ...
        ImageBubbleTheme.sentMessage.bubble.cornerRadius = KMChatMessageStyle.sentBubble.cornerRadius
        ImageBubbleTheme.receivedMessage.bubble.color = receivedMessageBackgroundColor
        ImageBubbleTheme.receivedMessage.bubble.cornerRadius = KMChatMessageStyle.receivedBubble.cornerRadius

        MessageTheme.sentMessage.message = KMChatMessageStyle.sentMessage
        MessageTheme.sentMessage.bubble.color = sentMessageBackgroundColor
        MessageTheme.sentMessage.bubble.cornerRadius = KMChatMessageStyle.sentBubble.cornerRadius
        MessageTheme.receivedMessage.message = KMChatMessageStyle.receivedMessage
        MessageTheme.receivedMessage.bubble.color = receivedMessageBackgroundColor
        MessageTheme.receivedMessage.bubble.cornerRadius = KMChatMessageStyle.receivedBubble.cornerRadius

        MessageTheme.receivedMessage.displayName = KMChatMessageStyle.displayName
        MessageTheme.receivedMessage.time = KMChatMessageStyle.time
        MessageTheme.sentMessage.time = KMChatMessageStyle.time
        setSentMessageStatus()

        /// Rich Message button primary color
        KMChatRichMessageStyle.primaryColor = appSettingsUserDefaults.getButtonPrimaryColor()
        
        RMCellConfiguration.hideSenderName = KMCellConfiguration.hideSenderName
    }

    func setSentMessageStatus() {
        let messageStatus = KMChatMessageStyle.messageStatus
        for (status, icon) in messageStatus.statusIcons {
            setStatus(statusIcon: icon, status: status)
        }
    }

    func setStatus(statusIcon: KMChatMessageStyle.StatusIcon, status: KMChatMessageStatus) {
        switch statusIcon {
    ...
    func reloadIfFormMessage(message: KMChatMessageModel, indexPath: IndexPath) {
        guard message.messageType == .form else { return }
        updateMessageAt(indexPath: indexPath)
    }

    func showDatePickerController(delegate: KMChatDatePickerButtonClickProtocol,
                                  identifier: String,
                                  position: Int,
                                  datePickerMode: UIDatePicker.Mode,
                                  localizedStringFileName: String) {
        let datePickerVC = KMChatFormDatePickerViewController(delegate: delegate,
...
extension KMChatConversationViewController: KMChatConversationViewModelDelegate {
    ...
    @objc open func loadingFinished(error _: Error?) {
        ...
        if let channelKey = viewModel.channelKey {
            KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.currentOpenedConversation, data: ["conversationId": Int(truncating: channelKey)])
        }
    }
    ...
    func containsOnlyNormalButton(message: KMChatMessageModel) -> Bool {
    ...
    func processFormTemplate(for message: KMChatMessageModel) {
        ...
        KMChatFormDataCache.shared.set(formData, for: key)
    }
    ...
    func rightNavbarButton() -> UIBarButtonItem? {
        ...
        let notificationSelector = #selector(KMChatConversationViewController.sendRightNavBarButtonSelectionNotification(_:))
    ...
    @objc func customButtonEvent(_ sender: AnyObject) {
        ...
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: KMChatNavigationItem.NSNotificationForConversationViewNavigationTap), object: self, userInfo: ["identifier": identifier])
    }
...
extension KMChatConversationViewController: KMChatCreateGroupChatAddFriendProtocol {
    func createGroupGetFriendInGroupList(friendsSelected _: [KMChatFriendViewModel], groupName: String, groupImgUrl: String?, friendsAdded: [KMChatFriendViewModel], groupDescription _: String?) {
...
extension KMChatConversationViewController: KMChatShareLocationViewControllerDelegate {
...
extension KMChatConversationViewController: KMChatLocationCellDelegate {
    func displayLocation(location: KMChatLocationPreviewViewModel) {
...
extension KMChatConversationViewController: KMChatAudioPlayerProtocol, KMChatVoiceCellProtocol {
...
extension KMChatConversationViewController: ALMQTTConversationDelegate {
    ...
    public func mqttConnectionClosed() {
        print("KMChatConversationVC mqtt connection closed.")
        ...
        print("Retrying MQTT connection in KMChatConversationVC after seconds:", String(format: "%.f", intervalSeconds))
...
extension KMChatConversationViewController: KMChatCustomPickerDelegate {
    func filesSelected(images: [UIImage], gifs: [String], videos: [String], caption: String) {
        ...
        for index in 0 ..< fileCount {
            if index < images.count {
                ...
                tableView.endUpdates()
                tableView.scrollToBottom(animated: false)
                guard let cell = tableView.cellForRow(at: indexPath) as? KMChatMyVideoCell else { return }
                guard ALDataNetworkConnection.checkDataNetworkAvailable() else {
                    let notificationView = ALNotificationView()
                    notificationView.noDataConnectionNotificationView()
                    return
