...
class KMChatPhotoPicker: NSObject, Localizable {
            ...
            for result in results {
                ...
                if provider.canLoadObject(ofClass: UIImage.self) {
                    ...
                    provider.loadFileRepresentation(forTypeIdentifier: UTType.movie.identifier) { url, error in
                        ...
                        } else if let url = url, let newURL = KMChatFileUtils().moveFileToDocuments(fileURL: url) {
...
@available(iOS 14, *)
extension KMChatPhotoPicker: PHPickerViewControllerDelegate {
