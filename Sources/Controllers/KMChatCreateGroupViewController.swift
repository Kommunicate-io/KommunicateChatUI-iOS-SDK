//
//  KMChatCreateGroupViewController.swift
...
protocol KMChatCreateGroupChatAddFriendProtocol: AnyObject {
    func createGroupGetFriendInGroupList(
        friendsSelected: [KMChatFriendViewModel],
        groupName: String,
        groupImgUrl: String?,
        friendsAdded: [KMChatFriendViewModel],
        groupDescription: String?
    )
}

final class KMChatCreateGroupViewController: KMChatBaseViewController, Localizable {
    enum KMChatAddContactMode: Localizable {
        case newChat
        case existingChat
    ...
    var groupList = [KMChatFriendViewModel]()
    var addedList = [KMChatFriendViewModel]()
    var addContactMode: KMChatAddContactMode = .newChat
    var groupDescriptionText: String?

    /// To be passed from outside for existing chat
    weak var groupDelegate: KMChatCreateGroupChatAddFriendProtocol?
    ...
    @IBOutlet fileprivate var txtfGroupName: KMChatGroupChatTextField!
    ...
    var viewModel: KMChatCreateGroupViewModel!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        tblParticipants.register(KMChatGroupMemberCell.self)
        tblParticipants.register(KMChatGroupDescriptionCell.self)
        tblParticipants.register(KMChatGroupHeaderTitleCell.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: KMChatGroupHeaderTitleCell.cellID)

        tblParticipants.showsVerticalScrollIndicator = false
        viewModel = KMChatCreateGroupViewModel(
            groupName: groupName,
            groupId: groupId,
            delegate: self,
    ...
    fileprivate func setupUI() {
        ...
        if let url = URL(string: groupProfileImgUrl) {
            let resource = Kingfisher.ImageResource(downloadURL: url, cacheKey: groupProfileImgUrl)
    ...
    @IBAction private func selectGroupImgPress(_: Any) {
        guard
            let vc = KMChatCustomCameraViewController.makeInstanceWith(delegate: self, and: configuration)
        else { return }
    ...
    func setCurrentGroupSelected(groupId: NSNumber,
                                 groupProfile: String?,
                                 delegate: KMChatCreateGroupChatAddFriendProtocol) {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "goToSelectFriendToAdd" {
            let selectParticipantViewController = segue.destination as? KMChatParticipantSelectionViewContoller
    ...
    private func changeUserRole(at index: Int, _ role: NSNumber) {
        ...
        let cell = tblParticipants.cellForItem(at: indexPath) as? KMChatGroupMemberCell
...
extension KMChatCreateGroupViewController: KMChatCreateGroupViewModelDelegate {
    ...
    func remove(at index: Int) {
        ...
        let removeAction = UIAlertAction(title: removeButton, style: .destructive, handler: { _ in
            let indexPath = IndexPath(row: index, section: CollectionViewSection.participants)
            let cell = self.tblParticipants.cellForItem(at: indexPath) as? KMChatGroupMemberCell
    ...
    func sendMessage(at index: Int) {
        let member = viewModel.rowAt(index: index)
        let conversationViewModel = KMChatConversationViewModel(
            contactId: member.id,
            channelKey: nil,
...
extension KMChatCreateGroupViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == CollectionViewSection.groupDescription {
            let cell: KMChatGroupDescriptionCell = tblParticipants.dequeueReusableCell(forIndexPath: indexPath)
            let groupViewModel = KMChatGroupDescriptionViewModel(channelKey: viewModel.groupId)
            ...
            let cell: KMChatGroupMemberCell = tblParticipants.dequeueReusableCell(forIndexPath: indexPath)
            cell.channelDetailConfig = configuration.channelDetail
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt path: IndexPath) -> CGSize {
        if path.section == CollectionViewSection.groupDescription {
            let groupViewModel = KMChatGroupDescriptionViewModel(channelKey: viewModel.groupId)
            let groupDescription = groupDescriptionText ?? groupViewModel.groupDescription()
            let height = KMChatGroupDescriptionCell.rowHeight(descrption: groupDescription, maxWidth: view.frame.width)
            return CGSize(width: view.frame.width - KMChatGroupDescriptionCell.Padding.DescriptionLabel.left + KMChatGroupDescriptionCell.Padding.DescriptionLabel.right, height: height)
        }
        return cellHeight()
    }
    ...
    func collectionView(_: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if indexPath.section == CollectionViewSection.groupDescription {
            let header = tblParticipants.dequeueReusableSupplementaryView(
                ...
                withReuseIdentifier: KMChatGroupHeaderTitleCell.cellID,
                for: indexPath
            ) as! KMChatGroupHeaderTitleCell
            ...
            let header = tblParticipants.dequeueReusableSupplementaryView(
                ofKind: UICollectionView.elementKindSectionHeader,
                withReuseIdentifier: "KMChatGroupHeaderTitleCell",
                for: indexPath
            ) as! KMChatGroupHeaderTitleCell
    ...
    func postNotificationForAddMember() {
        ...
        dic["Controller"] = self
        dic["AddMember"] = true
        NotificationCenter.default.post(name: Notification.Name(rawValue: KMChatNotification.createGroupAction), object: self, userInfo: dic)
    }
    ...
    public func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        if section == CollectionViewSection.groupDescription {
            let widthPadding = KMChatGroupHeaderTitleCell.Padding.HeaderTitleLabel.left + KMChatGroupHeaderTitleCell.Padding.HeaderTitleLabel.right

            let groupDescriptionHeaderTitle = localizedString(forKey: "GroupDescriptionHeaderTitle", withDefaultValue: SystemMessage.LabelName.GroupDescription, fileName: localizedStringFileName)

            let height = KMChatGroupHeaderTitleCell.rowHeight(descrption: groupDescriptionHeaderTitle, maxWidth: view.frame.width - widthPadding, font: configuration.channelDetail.participantHeaderTitle.font)
    ...
    private func cellHeight() -> CGSize {
        let height = KMChatGroupMemberCell.rowHeight()
    ...
    private func showGroupDescriptionViewController() {
        let groupDescriptionVC = KMChatGroupDescriptionController(channelKey: viewModel.groupId, configuration: configuration, isFromGroupCreate: addContactMode == .newChat)
    ...
    func showParticpentsTapActionView(options: [Options],
                                      row: Int,
                                      groupViewModel: KMChatCreateGroupViewModel) {
...
extension KMChatCreateGroupViewController: KMChatAddParticipantProtocol {
    ...
    func profileTappedAt(index: IndexPath) {
        ...
        let viewModel = KMChatConversationViewModel(
...
extension KMChatCreateGroupViewController: UITextFieldDelegate {
...
extension KMChatCreateGroupViewController: KMChatSelectParticipantToAddProtocol {
    func selectedParticipant(selectedList: [KMChatFriendViewModel], addedList: [KMChatFriendViewModel]) {
...
extension KMChatCreateGroupViewController {
    override func hideKeyboard() {
        let tap = UITapGestureRecognizer(
            target: self,
            action: #selector(KMChatCreateGroupViewController.dismissKeyboard)
        )
...
extension KMChatCreateGroupViewController: KMChatCustomCameraProtocol {
...
extension KMChatCreateGroupViewController: KMChatGroupDescriptionDelegate {
