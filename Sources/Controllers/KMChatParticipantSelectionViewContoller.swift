//
//  KMChatParticipantSelectionViewContoller.swift
...
class KMChatParticipantSelectionViewContoller: KMChatBaseViewController, Localizable {
    ...
    fileprivate var datasource = KMChatFriendDatasource()
...
//    private var friendDataService: FriendDataService?
    ...
    weak var selectParticipantDelegate: KMChatSelectParticipantToAddProtocol?
    ...
    func getAllFriends(completion: @escaping () -> Void) {
        if KMCoreSettings.isContactsGroupEnabled() {
            ...
            do {
                var models = [KMChatFriendViewModel]()
                ...
                if let fetchedContacts = try dbHandler?.execute(fetchReq as? NSFetchRequest<NSFetchRequestResult>) as? [DB_CONTACT] {
                    for dbContact in fetchedContacts {
                        ...
                        models.append(KMChatFriendViewModel(identity: contact))
                    }
    ...
    @IBAction func invitePress(_: Any) {
        ...
        var selectedFriendList = [KMChatFriendViewModel]()
    ...
    private func getAddedFriend(allFriendsInGroup: [KMChatFriendViewModel]) -> [KMChatFriendViewModel] {
        var addedFriendList = [KMChatFriendViewModel]()
    ...
    fileprivate func isInPreviousFriendGroup(fri: KMChatFriendViewModel) -> Bool {
...
extension KMChatParticipantSelectionViewContoller: UITableViewDelegate, UITableViewDataSource {
    func numberOfSections(in _: UITableView) -> Int {
        return 1
    }

    func tableView(_: UITableView, numberOfRowsInSection _: Int) -> Int {
        let state = KMChatDatasourceState(isInUsed: searchController.isActiveAndContainText())
...
//        return (array?.count)!
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "KMChatFriendContactCell", for: indexPath) as? KMChatFriendContactCell {
            let state = KMChatDatasourceState(isInUsed: searchController.isActiveAndContainText())
    ...
    private func keepTrackTappingNewlySelectedContact(fri: KMChatFriendViewModel) {
    ...
    private func handleTappingContact(friendViewModel: KMChatFriendViewModel) {
...
extension KMChatParticipantSelectionViewContoller: UISearchResultsUpdating, UISearchBarDelegate {
...
extension KMChatParticipantSelectionViewContoller: KMChatInviteButtonProtocol {
