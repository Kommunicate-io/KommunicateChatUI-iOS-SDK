...
protocol KMChatHTTPManagerUploadDelegate: AnyObject {
    func dataUploaded(task: KMChatUploadTask)
    func dataUploadingFinished(task: KMChatUploadTask)
}

protocol KMChatHTTPManagerDownloadDelegate: AnyObject {
...
class KMChatHTTPManager: NSObject {
    static let shared = KMChatHTTPManager()
    weak var downloadDelegate: KMChatHTTPManagerDownloadDelegate?
    weak var uploadDelegate: KMChatHTTPManagerUploadDelegate?
    var uploadCompleted: ((_ responseDict: Any?, _ task: KMChatUploadTask) -> Void)?
    var downloadCompleted: ((_ task: KMChatDownloadTask) -> Void)?

    var length: Int64 = 0
    var buffer = NSMutableData()
    var session: URLSession?
    var uploadTask: KMChatUploadTask?
    var downloadTask: KMChatDownloadTask?
    ...
    func downloadAttachment(task: KMChatDownloadTask) {
        downloadTask = task
    ...
    func downloadImage(task: KMChatDownloadTask) {
        downloadTask = task
        guard let urlString = task.urlString, let fileName = task.fileName, let identifier = task.identifier else { return }
        let componentsArray = fileName.components(separatedBy: ".")
        let fileExtension = componentsArray.last
    ...
    func uploadAttachment(task: KMChatUploadTask) {
        uploadTask = task
...
extension KMChatHTTPManager: URLSessionDataDelegate {
