//
//  KMChatConversationViewModel.swift
//
//
...
#endif
public protocol KMChatConversationViewModelDelegate: AnyObject {
    func loadingStarted()
...
open class KMChatConversationViewModel: NSObject, Localizable {
    ...
    public weak var delegate: KMChatConversationViewModelDelegate?
    ...
    var modelsToBeAddedAfterDelay: [KMChatMessageModel] = []
    ...
    open var messageModels: [KMChatMessageModel] = []
    ...
    private var selectedMessageForReply: KMChatMessageViewModel?
    ...
    public required init(
        ...
        KMChatConversationViewModel.lastSentMessage = nil
    }
    ...
    public func addToWrapper(message: KMCoreMessage) {
        ...
        if message.isMyMessage {
            KMChatConversationViewModel.lastSentMessage = message
        }
    }
    ...
    open func friends() -> [KMChatFriendViewModel] {
        let alChannelService = KMCoreChannelService()
        ...
        let models = alContacts.filter { $0?.userId != KMCoreUserDefaultsHandler.getUserId() }.map { KMChatFriendViewModel(identity: $0!) }
    ...
    open func messageForRow(indexPath: IndexPath) -> KMChatMessageViewModel? {
        guard indexPath.section < messageModels.count, indexPath.section >= 0 else { return nil }
        return messageModels[indexPath.section]
    }

    open func quickReplyDictionary(message: KMChatMessageViewModel?, indexRow row: Int) -> [String: Any]? {
    ...
    open func messageForRow(identifier: String) -> KMChatMessageViewModel? {
    ...
    open func heightForRow(indexPath: IndexPath, cellFrame _: CGRect, configuration: KMChatConfiguration) -> CGFloat {
        ...
        case .text, .html, .email:
            guard !configuration.isLinkPreviewDisabled, messageModel.messageType == .text, KMChatLinkPreviewManager.extractURLAndAddInCache(from: messageModel.message, identifier: messageModel.identifier) != nil else {
                ...
                if messageModel.isMyMessage {
                    let height = KMChatMyMessageCell.rowHeigh(viewModel: messageModel, width: maxWidth, displayNames: { userIds in
                        self.displayNames(ofUserIds: userIds)
                    })
                    return height.cached(with: cacheIdentifier)
                } else {
                    let height = KMChatFriendMessageCell.rowHeigh(viewModel: messageModel, width: maxWidth, displayNames: { userIds in
                        self.displayNames(ofUserIds: userIds)
                    })
                    return height.cached(with: cacheIdentifier)
                }
            ...
            if messageModel.isMyMessage {
                let height = KMChatMyLinkPreviewCell.rowHeigh(viewModel: messageModel, width: maxWidth, displayNames: { userIds in
                    self.displayNames(ofUserIds: userIds)
                })
                return height.cached(with: cacheIdentifier)
            } else {
                let height = KMChatFriendLinkPreviewCell.rowHeigh(viewModel: messageModel, width: maxWidth, displayNames: { userIds in
                    self.displayNames(ofUserIds: userIds)
                })
                return height.cached(with: cacheIdentifier)
            }
        ...
        case .photo:
            if messageModel.isMyMessage {
                if messageModel.ratio < 1 {
                    let heigh = KMChatMyPhotoPortalCell.rowHeigh(viewModel: messageModel, width: maxWidth)
                    return heigh.cached(with: cacheIdentifier)
                } else {
                    let heigh = KMChatMyPhotoLandscapeCell.rowHeigh(viewModel: messageModel, width: maxWidth)
                    return heigh.cached(with: cacheIdentifier)
                }
            } else {
                if messageModel.ratio < 1 {
                    let heigh = KMChatFriendPhotoPortalCell.rowHeigh(viewModel: messageModel, width: maxWidth)
                    return heigh.cached(with: cacheIdentifier)
                } else {
                    let heigh = KMChatFriendPhotoLandscapeCell.rowHeigh(viewModel: messageModel, width: maxWidth)
                    return heigh.cached(with: cacheIdentifier)
                }
            }
        case .voice:
            ...
            if messageModel.isMyMessage {
                height = KMChatVoiceCell.rowHeigh(viewModel: messageModel, width: maxWidth)
            } else {
                height = KMChatFriendVoiceCell.rowHeigh(viewModel: messageModel, width: maxWidth)
            }
            return height.cached(with: cacheIdentifier)
        case .information:
            let height = KMChatInformationCell.rowHeigh(viewModel: messageModel, width: maxWidth)
            return height.cached(with: cacheIdentifier)
        case .location:
            return (messageModel.isMyMessage ? KMChatMyLocationCell.rowHeigh(viewModel: messageModel, width: maxWidth).cached(with: cacheIdentifier) : KMChatFriendLocationCell.rowHeigh(viewModel: messageModel, width: maxWidth)).cached(with: cacheIdentifier)
        case .video:
            var height: CGFloat = 0
            if messageModel.isMyMessage {
                height = KMChatMyVideoCell.rowHeigh(viewModel: messageModel, width: maxWidth)
            } else {
                height = KMChatFriendVideoCell.rowHeigh(viewModel: messageModel, width: maxWidth)
            }
            return height.cached(with: cacheIdentifier)
        case .cardTemplate:
            if messageModel.isMyMessage {
                return
                    KMChatMyGenericCardMessageCell
                        .rowHeigh(viewModel: messageModel, width: maxWidth)
                        .cached(with: cacheIdentifier)
            } else {
                return
                    KMChatFriendGenericCardMessageCell
                        .rowHeigh(viewModel: messageModel, width: maxWidth)
                        .cached(with: cacheIdentifier)
            }
        case .faqTemplate:
        ...
        case .listTemplate:
            if messageModel.isMyMessage {
                return
                    KMChatMyMessageListTemplateCell
                        .rowHeight(viewModel: messageModel, maxWidth: UIScreen.main.bounds.width)
                        .cached(with: cacheIdentifier)
            } else {
                return
                    KMChatFriendMessageListTemplateCell
                        .rowHeight(viewModel: messageModel, maxWidth: UIScreen.main.bounds.width)
                        .cached(with: cacheIdentifier)
            }
        case .document:
            if messageModel.isMyMessage {
                return
                    KMChatMyDocumentCell
                        .rowHeigh(viewModel: messageModel, width: maxWidth)
                        .cached(with: cacheIdentifier)
            } else {
                return
                    KMChatFriendDocumentCell
                        .rowHeigh(viewModel: messageModel, width: maxWidth)
                        .cached(with: cacheIdentifier)
            }
        case .imageMessage:
    ...
    open func getContextTitleData() -> KMChatContextTitleDataType? {
        ...
        do {
            ...
            if let json = jsonResult as? [String: Any],
               let templates = json["templates"] as? [Any] {
                ...
                var templateModels: [KMChatTemplateMessageModel] = []
                for element in templates {
                    if let template = element as? [String: Any],
                       let model = KMChatTemplateMessageModel(json: template) {
    ...
    open func downloadAttachment(message: KMChatMessageViewModel, view: UIView) {
        ...
        if message.fileMetaInfo!.name.hasPrefix(awsEncryptionPrefix) {
          KMCoreMessageClientService().downloadImageUrlV2(message.fileMetaInfo?.blobKey, isS3URL: true) { fileUrl, error in
               ...
               let httpManager = KMChatHTTPManager()
               httpManager.downloadDelegate = view as? KMChatHTTPManagerDownloadDelegate
               let task = KMChatDownloadTask(downloadUrl: fileUrl, fileName: message.fileMetaInfo?.name)
        ...
        if let url = message.fileMetaInfo?.url,
           !serviceEnabled {
            let httpManager = KMChatHTTPManager()
            httpManager.downloadDelegate = view as? KMChatHTTPManagerDownloadDelegate
            let task = KMChatDownloadTask(downloadUrl: url, fileName: message.fileMetaInfo?.name)
            task.identifier = message.identifier
            task.totalBytesExpectedToDownload = message.size
    ...
    open func addMessagesToList(_ messageList: [Any]) {
        ...
        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.messageReceive, data: ["messageList": sortedArray])
    ...
    open func updateGroup(groupName: String, groupImage: String?, friendsAdded: [KMChatFriendViewModel]) {
    ...
    open func send(message: String, isOpenGroup: Bool = false, metadata: [AnyHashable: Any]?) {
        ...
        if isOpenGroup {
            ...
            messageClientService.sendMessage(alMessage.dictionary(), withCompletionHandler: { _, error in
                ...
                KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.messageSend, data: ["message": alMessage])
    ...
    func isValidReply(message: KMCoreMessage) -> Bool {
        ...
        do {
            return try KMChatRegexValidator.matchPattern(text: messageText, pattern: regex)
        } catch {
    ...
    open func send(photo: UIImage, metadata: [AnyHashable: Any]?, caption: String) -> (KMCoreMessage?, IndexPath?) {
        ...
        let filePath = KMChatFileUtils().saveImageToDocDirectory(image: photo)
    ...
    open func uploadVideo(view: UIView, indexPath: IndexPath) {
        ...
        let uploadManager = KMChatVideoUploadManager()
        uploadManager.uploadDelegate = view as? KMChatHTTPManagerUploadDelegate
    ...
    open func updateMessageModelAt(indexPath: IndexPath, data: Data) {
        ...
        messageModels[indexPath.section] = message as! KMChatMessageModel
    ...
    open func uploadAudio(alMessage: KMCoreMessage, indexPath: IndexPath) {
        ...
        clientService.sendPhoto(forUserInfo: alMessage.dictionary(), withCompletion: {
            ...
            let task = KMChatUploadTask(url: url, fileName: alMessage.fileMeta.name)
            ...
            let downloadManager = KMChatHTTPManager()
    ...
    open func uploadImage(view: UIView, indexPath: IndexPath) {
        ...
        clientService.sendPhoto(forUserInfo: alMessage.dictionary(), withCompletion: {
            ...
            task.groupdId = alMessage.groupId.stringValue
            let downloadManager = KMChatHTTPManager()
            downloadManager.uploadDelegate = view as? KMChatHTTPManagerUploadDelegate
            downloadManager.uploadAttachment(task: task)
            downloadManager.uploadCompleted = { [weak self] responseDict, task in
                if task.uploadError == nil, task.completed {
                    self?.uploadAttachmentCompleted(responseDict: responseDict, indexPath: indexPath)
    ...
    open func selected(template: KMChatTemplateMessageModel, metadata: [AnyHashable: Any]?) {
    ...
    open func setSelectedMessageToReply(_ message: KMChatMessageViewModel) {
        selectedMessageForReply = message
    }

    open func getSelectedMessageToReply() -> KMChatMessageViewModel? {
    ...
    open func getIndexpathFor(message: KMChatMessageModel) -> IndexPath? {
    ...
    func currentConversationProfile(completion: @escaping (KMChatConversationProfile?) -> Void) {
        if channelKey != nil {
    ...
    func conversationProfileFrom(contact: ALContact?, channel: KMCoreChannel?) -> KMChatConversationProfile {
        var conversationProfile = KMChatConversationProfile()
        ...
        conversationProfile.status = KMChatConversationProfile.Status(isOnline: contact.connected, lastSeenAt: contact.lastSeenAt)
        return conversationProfile
    }
    ...
    func removeMessageForHidePostCTA(messages: [KMChatMessageModel]) {
        guard let lastSentMessageTime = KMChatConversationViewModel.lastSentMessage?.createdAtTime,
    ...
    private func addMembersToGroup(users: [KMChatFriendViewModel], completion: @escaping (Bool) -> Void) {
        guard let groupId = groupKey() else { return }
        let alchanneService = KMCoreChannelService()
    ...
    private func getGenericCardTemplateFor(message: KMChatMessageViewModel) -> KMChatGenericCardTemplate? {
        guard
            let metadata = message.metadata,
            let payload = metadata["payload"] as? String
        else { return nil }
        do {
            let cards = try JSONDecoder().decode([KMChatGenericCard].self, from: payload.data)
            let cardTemplate = KMChatGenericCardTemplate(cards: cards)
