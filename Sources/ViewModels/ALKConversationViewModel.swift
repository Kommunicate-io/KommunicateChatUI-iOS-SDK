    ...
    open func sync(message: ALMessage) {
        ...
        if let groupId = message.groupId, groupId != self.channelKey {
            ...
            notificationView?.showNativeNotificationWithcompletionHandler({
                ...
                self.channelKey = nil
                self.isFirstTime = true
                let messageName = message.name.count > 0 ? message.name : self.localizedString(forKey: "NoNameMessage", withDefaultValue: SystemMessage.NoData.NoName, config: self.configuration)
