//
//  KMChatTemplateMessagesViewModel.swift
...
open class KMChatTemplateMessagesViewModel: NSObject {
    open var messageTemplates: [KMChatTemplateMessageModel]
    ...
    public init(messageTemplates: [KMChatTemplateMessageModel]) {
    ...
    open func getTemplateForItemAt(row: Int) -> KMChatTemplateMessageModel? {
        guard row >= 0, row < messageTemplates.count else {
            return nil
        }
