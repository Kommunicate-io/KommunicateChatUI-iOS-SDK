...
class KMChatAttatchmentView: UIView {
    let loadingIndicator = KMChatLoadingIndicator(frame: .zero, color: UIColor.gray)
    ...
    func download() {
        ...
        if let url = messageObject.fileMetaInfo?.url,
           !serviceEnabled {
            ...
            let task = KMChatDownloadTask(downloadUrl: url, fileName: messageObject.fileMetaInfo?.name)
        ...
        KMCoreMessageClientService().downloadImageUrlV2(messageObject.fileMetaInfo?.blobKey, isS3URL: messageObject.fileMetaInfo?.url != nil) { fileUrl, error in
            ...
            let httpManager = KMChatHTTPManager()
            httpManager.downloadDelegate = self
            let task = KMChatDownloadTask(downloadUrl: fileUrl, fileName: messageObject.fileMetaInfo?.name)
            task.identifier = messageObject.identifier
            task.totalBytesExpectedToDownload = messageObject.size
...
extension KMChatAttatchmentView: KMChatHTTPManagerDownloadDelegate {
