...
class KMChatDocumentCell: KMChatChatBaseCell<KMChatMessageViewModel> {
    enum CommonPadding {
        enum FrameUIView {
    ...
    let frontView: KMChatTappableView = {
    ...
    @objc func openWKWebView(gesture _: UITapGestureRecognizer) {
        ...
        let docViewController = KMChatDocumentViewerController()
    ...
    override func update(viewModel: KMChatMessageViewModel) {
        ...
        fileNameLabel.text = KMChatFileUtils().getFileName(filePath: viewModel.filePath, fileMeta: viewModel.fileMetaInfo)

        let size = KMChatFileUtils().getFileSize(filePath: viewModel.filePath, fileMetaInfo: viewModel.fileMetaInfo) ?? ""

        let fileType = KMChatFileUtils().getFileExtenion(filePath: viewModel.filePath, fileMeta: viewModel.fileMetaInfo)
...
extension KMChatDocumentCell: KMChatHTTPManagerUploadDelegate {
...
extension KMChatDocumentCell: KMChatHTTPManagerDownloadDelegate {
