...
open class KMChatChatBar: UIView, Localizable {
    var configuration: KMChatConfiguration!

    public var chatBarConfiguration: KMChatChatBarConfiguration {
    ...
    open lazy var soundRec: KMChatAudioRecorderView = {
        let view = KMChatAudioRecorderView(frame: CGRect.zero, configuration: self.configuration)
    ...
    open lazy var placeHolder: UITextView = {
        let view = UITextView()
        view.setFont(KMChatChatBarConfiguration.TextView.placeholder.font)
        view.setTextColor(KMChatChatBarConfiguration.TextView.placeholder.text)
    ...
    open var languageSelectionButton: UIButton = {
        ...
        let tintColor = KMChatAppSettingsUserDefaults().getAttachmentIconsTintColor()
    ...
    lazy var defaultTextAttributes: [NSAttributedString.Key: Any] = {
        ...
        let attrs = [
            ...
            NSAttributedString.Key.font: KMChatChatBarConfiguration.TextView.text.font,
            NSAttributedString.Key.foregroundColor: KMChatChatBarConfiguration.TextView.text.text
        ]
        return attrs
    }() {
        didSet {
    ...
    public required init(frame: CGRect, configuration: KMChatConfiguration) {
    ...
    private func setupConstraints(
        ...
        func buttonsForOptions(_ options: KMChatChatBarConfiguration.AttachmentOptions) -> [UIButton] {
    ...
    /// changed based on `KMChatChatBarConfiguration`s `optionsToShow`
    /// value.
...
extension KMChatChatBar: UITextViewDelegate {
...
extension KMChatChatBar: KMChatAudioRecorderProtocol {
    public func startRecordingAudio() {
        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.voiceButtonClicked, data: ["currentState": KMVoiceRecordingState.started])
        changeButton()
    ...
    public func finishRecordingAudio(soundData: NSData) {
        KMChatCustomEventHandler.shared.publish(triggeredEvent: KMCustomEvent.voiceButtonClicked, data: ["currentState": KMVoiceRecordingState.stopped])
        textView.resignFirstResponder()
...
public class KMExtendedTouchAreaButton: UIButton {
    
    public var extraTouchArea: UIEdgeInsets = UIEdgeInsets(top: -15, left: -15, bottom: -15, right: -15)
