...
class KMChatMyLinkPreviewCell: KMChatLinkPreviewBaseCell {
    ...
    static var bubbleViewRightPadding: CGFloat = {
        ...
        guard KMChatMessageStyle.sentBubble.style == .edge else {
            return KMChatMessageStyle.sentBubble.widthPadding
        }
        return KMChatMessageStyle.sentBubble.widthPadding + 5
    }()
    ...
    override func setupViews() {
        ...
        NSLayoutConstraint.activate([
            ...
            emailTopView.leadingAnchor.constraint(
                ...
                constant: KMChatMessageStyle.sentBubble.widthPadding
            ),
            emailTopView.trailingAnchor.constraint(
                equalTo: bubbleView.trailingAnchor,
                constant: -KMChatMyMessageCell.bubbleViewRightPadding
            ),
            emailTopHeight,
    ...
    override open func setupStyle() {
        super.setupStyle()
        messageView.setStyle(KMChatMessageStyle.sentMessage)
        bubbleView.setStyle(KMChatMessageStyle.sentBubble, isReceiverSide: false)
        setStatusStyle(statusView: stateView, KMChatMessageStyle.messageStatus)
    }

    override open func update(viewModel: KMChatMessageViewModel) {
        super.update(
            viewModel: viewModel,
            messageStyle: KMChatMessageStyle.sentMessage,
            mentionStyle: KMChatMessageStyle.sentMention
        )
    ...
    class func rowHeigh(viewModel: KMChatMessageViewModel,
                        width: CGFloat,
                        displayNames: ((Set<String>) -> ([String: String]?))?) -> CGFloat {
        /// Calculating messageHeight
        let leftSpacing = Padding.BubbleView.left + KMChatMessageStyle.sentBubble.widthPadding
        ...
        let messageHeight = super
            .messageHeight(
                viewModel: viewModel,
                width: messageWidth,
                font: KMChatMessageStyle.sentMessage.font,
                mentionStyle: KMChatMessageStyle.sentMention,
        ...
        let linkURL = KMChatLinkPreviewManager.extractURLAndAddInCache(from: viewModel.message, identifier: viewModel.identifier)
        ...
        if isReplyMessage, linkURL != nil {
            let linkViewHeight = KMChatLinkView.height() + Padding.LinkView.top
            ...
            return messageHeight + heightPadding + KMChatLinkView.height() + Padding.LinkView.top
        }
    ...
    override func menuWillShow(_ sender: Any) {
        ...
        if KMChatMessageStyle.sentBubble.style == .edge {
            bubbleView.image = bubbleView.imageBubble(
                for: KMChatMessageStyle.sentBubble.style,
