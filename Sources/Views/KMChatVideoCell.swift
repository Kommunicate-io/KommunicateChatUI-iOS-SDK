...
#endif
class KMChatVideoCell: KMChatChatBaseCell<KMChatMessageViewModel>,
    ...
    override func setupStyle() {
        ...
        fileSizeLabel.setStyle(KMChatMessageStyle.time)
    }
    ...
    override func setupViews() {
        ...
        downloadButton.addTarget(self, action: #selector(KMChatVideoCell.downloadButtonAction(_:)), for: UIControl.Event.touchUpInside)
        uploadButton.addTarget(self, action: #selector(KMChatVideoCell.uploadButtonAction(_:)), for: .touchUpInside)
        playButton.addTarget(self, action: #selector(KMChatVideoCell.playButtonAction(_:)), for: .touchUpInside)
        ...
        captionLabel.layout {
            ...
            $0.bottom == contentView.bottomAnchor - KMChatVideoCell.bottomPadding()
        }
    }
    ...
    @objc private func playButtonAction(_: UIButton) {
        ...
        let mediaViewerViewModel = KMChatMediaViewerViewModel(
        ...
        let nav = KMChatBaseNavigationViewController(rootViewController: mediaVC)
    ...
    func loadThumbnail(isDownloaded: Bool = false, filePath: String? = nil) {
        ...
        guard let thumbnailPath = metadata.thumbnailFilePath else {
            KMCoreMessageClientService().downloadImageThumbnailUrlV2(metadata.thumbnailUrl, isS3URL: metadata.url != nil, blobKey: updatedBlobKey) { url, error in
                ...
                let task = KMChatDownloadTask(downloadUrl: url, fileName: metadata.name)
...
extension KMChatVideoCell: KMChatHTTPManagerUploadDelegate {
    func dataUploaded(task: KMChatUploadTask) {
    ...
    func dataUploadingFinished(task: KMChatUploadTask) {
...
extension KMChatVideoCell: KMChatHTTPManagerDownloadDelegate {
    func dataDownloaded(task: KMChatDownloadTask) {
