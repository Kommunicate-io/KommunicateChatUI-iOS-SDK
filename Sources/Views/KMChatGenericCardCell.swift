...
open class KMChatGenericCardCollectionView: KMChatIndexedCollectionView {
    open var cardTemplate: [KMCardTemplate]?

    override open func setMessage(viewModel: KMChatMessageViewModel) {
        super.setMessage(viewModel: viewModel)
        // set card template
        guard let templates = KMChatGenericCardCollectionView.getCardTemplate(message: viewModel) else { return }
        cardTemplate = templates
    }

    override open class func rowHeightFor(message: KMChatMessageViewModel, width: CGFloat) -> CGFloat {
        ...
        let maxHeight = template
            .map { KMChatGenericCardCell.rowHeight(card: $0, maxWidth: width) }
            .max { $0 < $1 }
        let defaultHeight = KMChatGenericCardCell.rowHeight(card: card, maxWidth: width)
        return maxHeight ?? defaultHeight
    }

    class func getCardTemplate(message: KMChatMessageViewModel) -> [KMCardTemplate]? {
        guard
            let metadata = message.metadata,
            let templateId = metadata["templateId"] as? String
        else { return nil }
        switch templateId {
        case KMActionableMessageType.cardTemplate.rawValue:
            do {
        ...
        case KMActionableMessageType.genericCard.rawValue:
            do {
                let cards = try TemplateDecoder.decode([KMChatGenericCard].self, from: metadata)
                var templates = [KMCardTemplate]()
                for card in cards {
...
open class KMChatGenericCardCell: UICollectionViewCell {
    public enum Font {
    ...
    open class func rowHeight(card: KMCardTemplate, maxWidth: CGFloat) -> CGFloat {
        ...
        let titleHeight = textHeight(card.title, size: titleConstraint, font: Font.title) * CGFloat(KMChatGenericCardCell.Config.titleMaxLines)

        let subtitleConstraint = CGSize(width: maxWidth, height: Font.subtitle.lineHeight)
        let subtitleHeight = textHeight(card.subtitle, size: subtitleConstraint, font: Font.subtitle) * CGFloat(KMChatGenericCardCell.Config.subtitleMaxLines)
    ...
    open func update(card: KMCardTemplate) {
        self.card = card
        setTitle(card.title)
    ...
    private func setOverlayText(_ header: KMCardTemplate.Header?) {
    ...
    private func setCoverImage(_ header: KMCardTemplate.Header?) {
    ...
    private func setRatingLabel(_ card: KMCardTemplate) {
    ...
    private func setDescription(_ card: KMCardTemplate) {
        ...
        let height = description.rectWithConstrainedSize(descriptionConstraint, font: Font.description).height.rounded(.up) * CGFloat(KMChatGenericCardCell.Config.descriptionMaxLines)
