...
/// KMChatNavigationItem class is used for creating a Navigation bar items
public struct KMChatNavigationItem {
...
public extension KMChatNavigationItem {
    ///  Convenience initializer for creating `KMChatNavigationItem` with text.
    ///
    /// - Parameters:
    ...
    ///  Convenience initializer for creating `KMChatNavigationItem` with an icon.
    ///
    /// - Parameters:
    ///   - identifier: A unique identifier, that will be part of the tap
    ///                 notification for identifying the tapped button.
    ...
    func barButton(target: Any, action: Selector) -> UIBarButtonItem? {
        guard let image = buttonImage else {
            ...
            if identifier == faqIdentifier {
                ...
                let btn = KMExtendedTouchAreaButton(frame: CGRect(x: 0.0, y: 0.0, width: 50.0, height: 30.0) )
