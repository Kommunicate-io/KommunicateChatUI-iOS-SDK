...
@objc class ALChatManager: NSObject {
    ...
    @objc func connectUser(_ alUser: ALUser, completion: @escaping (_ response: ALRegistrationResponse?, _ error: NSError?) -> Void) {
        ...
        registerUserClientService.initWithCompletion(alUser, withCompletion: { response, error in
            ...
            guard let response = response else {
                let apiError = NSError(domain: "Kommunicate", code: 0, userInfo: [NSLocalizedDescriptionKey: "Api error while registering to kommunicate"])
                completion(nil, apiError as NSError?)
                return
            }
            guard response.isRegisteredSuccessfully() else {
                let message = response.message ?? "Api error while registering to kommunicate"
                let errorResponse = NSError(domain: "Kommunicate", code: 0, userInfo: [NSLocalizedDescriptionKey: message])
    ...
    func setApplicationBaseUrl() {
        guard let dict = Bundle.main.infoDictionary?["KOMMUNICATE_PRODUCTION"] as? [AnyHashable: Any] else {
            return
        }
    ...
    /// This method is used for updating APN's device token to kommunicate server.
    /// - Parameters:
