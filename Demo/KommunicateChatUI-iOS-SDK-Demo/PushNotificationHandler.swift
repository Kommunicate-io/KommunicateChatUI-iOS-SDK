...
class PushNotificationHandler {
    ...
    func launchIndividualChatWith(notificationData: NotificationHelper.NotificationData) {
        ...
        if NotificationHelper().isKMChatUIVCAtTop() {
            NotificationHelper().handleNotificationTap(notificationData)
        } else {
            print("Kommunicate Controller not on top. Launch chat using helper methods.")
            handleNotificationWhenKMNotOnTop(notificationData)
        }
    }

    private func handleNotificationWhenKMNotOnTop(_ notificationData: NotificationHelper.NotificationData) {
